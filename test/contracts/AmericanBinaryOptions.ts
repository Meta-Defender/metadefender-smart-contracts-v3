...
import { TestAmericanBinaryOptions } from '../../typechain-types';
...
describe('AmericanBinaryOptions - values', () => {
    let account: Signer;
    let testAmericanBinaryOptions: TestAmericanBinaryOptions;

    beforeEach(async () => {
        const signers = await ethers.getSigners();
        account = signers[0];
        testAmericanBinaryOptions = (await (
            await ethers.getContractFactory('TestAmericanBinaryOptions')
        )
            .connect(account)
            .deploy()) as TestAmericanBinaryOptions;
    });
    ...
    describe('ln', async () => {
        ...
        it('should provide a correct value for a number of cases', async () => {
            for (const val of lnTests) {
                const result = await testAmericanBinaryOptions.ln_pub(
    ...
    describe('n', async () => {
        ...
        it('should provide a correct value for a number of cases', async () => {
            for (const val of nTests) {
                const result = await testAmericanBinaryOptions.stdNormalCDF_pub(
                    toBN(val.toString()).mul(1e9),
                );
        ...
        it('should provide a correct value for a number of cases', async () => {
            for (const val of nTests) {
                let result = await testAmericanBinaryOptions.erf_pub(
                    toBN(val.toString()).mul(1e9),
                );
                const expected = erf(val);
                if (Math.abs(Number(result.toString())) < 10000000000000) {
    ...
    describe('exp', async () => {
        ...
        it('should provide a correct value for a number of cases', async () => {
            for (const val of expTests) {
                ...
                const result = await testAmericanBinaryOptions.exp_pub(
                    toBN(val.toString()).mul(1e9),
                );
                assertCloseToPercentage(result, toBN(expected.toString()));
            }
        ...
        it('should revert for values greater than 100', async () => {
            await expect(
                testAmericanBinaryOptions.exp_pub(toBN('100').mul(1e9).add(1)),
            ).revertedWith('cannot handle exponents greater than 100');
        });
    });

    describe('sqrt', async () => {
        const sqrtTests = [
            0, 0.00005, 0.0005, 0.005, 0.05, 0.5, 1, 2, 100, 100000, 250000,
            500000, 1000000, 9848575333047, 1e10, 1e12, 1e14, 1e16, 1e18, 1e19,
        ];
        it('should provide a correct value for a number of cases', async () => {
            for (const val of sqrtTests) {
                const result = await testAmericanBinaryOptions.sqrt_pub(
                    toBN(val.toString()),
                );
    ...
    describe('d1d2', async () => {
        const timeToExp = [
            // 0,
            0.00005, 0.0005, 0.005, 0.05, 0.5, 1, 2, 100, 100000, 250000,
            500000, 1000000, 9848575333047, 1e10, 1e12, 1e14, 1e16, 1e18, 1e19,
        ];
        it('calculates d1 with respect to changes in time to expiry', async () => {
            for (const val of timeToExp) {
                ...
                const rate = toBN('0.1');
                const result = await testAmericanBinaryOptions.d1d2_pub(
                    toBN(val.toString()).mul(1e9),
                    volatility.mul(1e9),
                    spot.mul(1e9),
                    strike.mul(1e9),
    ...
    describe('optionPrices - spot == strike', async () => {
        ...
        it('calculates optionPrices with respect to changes in time to expiry', async () => {
            for (const val of timeToExp) {
                ...
                const strike = toBN('2000');
                const rate = toBN('0.1');
                const result = await testAmericanBinaryOptions.optionPrices(
                    val,
                    volatility,
                    spot,
                    strike,
    ...
    describe('american binary option', async () => {
        // const timeToExp = [
        //     // 0,
        //     1, 2, 100, 100000, 250000, 500000, 1000000, 9848575333047,
        // ];
        ...
        it('calculates optionPrices with respect to changes in time to expiry', async () => {
            for (const val of timeToExp) {
                const volInDecimal = 0.5;
                const spotInDecimal = 1000;
                const strikeInDecimal = 2000;
                const rateInDecimal = 0.1;
                const result =
                    await testAmericanBinaryOptions.americanBinaryOptionPrices(
                        val,
                        toBN(volInDecimal.toString()),
                        toBN(spotInDecimal.toString()),
                        toBN(strikeInDecimal.toString()),
                        toBN(rateInDecimal.toString()),
                    );
                const tAnnualised = val / YEAR_SEC;
                const expectedCall = americanBinaryOptions(
                    tAnnualised,
                    volInDecimal,
                    spotInDecimal,
                    strikeInDecimal,
...
describe('BlackScholes', () => {
    ...
    let americanBinaryOptions: TestAmericanBinaryOptions;
    let account1: Signer;
    ...
    beforeEach(async () => {
        const bsFactory = await ethers.getContractFactory(
            'TestAmericanBinaryOptions',
        );
        americanBinaryOptions = (await bsFactory
            .connect(account1)
            .deploy()) as TestAmericanBinaryOptions;
    ...
    describe('Option Pricing and Greeks', async () => {
        describe('d1d2', async () => {
            it('Produces the correct result on example data', async () => {
                ...
                const d1d2 = await americanBinaryOptions.d1d2_pub(...args);
            ...
            it('swapping spot and strike with zero rate yields swapped and negated results', async () => {
                ...
                const d1d2_1 = await americanBinaryOptions.d1d2_pub(...args);

                args = bsInputs(t, vol, strike, spot, rate, true);
                const d1d2_2 = await americanBinaryOptions.d1d2_pub(...args);
        ...
        describe('Prices', async () => {
            ...
            it('Basic values give the proper result', async () => {
                ...
                const prices = await americanBinaryOptions.optionPrices(
                    ...args,
                );
                const call = prices[0];
                const put = prices[1];
            ...
            it('Inverting spot and strike with no risk free rate swaps the prices', async () => {
                ...
                const pricesA = await americanBinaryOptions.optionPrices(
                    ...args,
                );
                ...
                const pricesB = await americanBinaryOptions.optionPrices(
                    ...args,
                );
...
function bsInputs(
    ...
    const c = precise
        ? (x: number) => toBN(x.toString()).mul(1e9)
        : (x: number) => toBN(x.toString());
    return [
        precise ? c(annualise(tSeconds)) : BigNumber.from(tSeconds),
        c(vol),
        c(spot),
        c(strike),
        c(rate),
